...
export default function Ratel() {
    ...
    return (
        <section className="my-5 text-center text-md-start">
            ...
            <div className="d-flex justify-content-between mt-5 p-4 bg-light rounded container text-center text-md-start">
                {images.map((data) => {return <div className="d-flex ">
                    <Image key = {data.key} src = {data.src} className=" me-2 rounded" width={80} height={80} />
                </div>  })}
            </div>
        </section>
    )
